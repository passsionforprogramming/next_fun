...
/**
 * Seeds advocate data into the database
 */
export async function seedAdvocates() {
  console.log('Seeding advocates...');
  
  try {
    // Insert all advocates in a batch operation
    // Convert phoneNumber to string to match database schema expectations
    const advocatesWithStringPhoneNumbers = advocateData.map(advocate => ({
      ...advocate,
      phoneNumber: advocate.phoneNumber.toString()
    }));
    
    const result = await db.insert(advocates).values(advocatesWithStringPhoneNumbers);
